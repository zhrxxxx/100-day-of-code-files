#mutiple rings change colour
import time
# 1. rings:
# a way to count the rings
numRings = 99; # arbitrary (but small changes can make a significant aesthetic impact!)
...
def draw():
  global numRings, ring;
  background(255,255,255); # that pinkish background colour
...
class Ring(object):
  ...
  def display(self):
    ...
    if (self.on == True):
      ...
      for i in range (3): 
        self.colour = color(random(155), random(155), random(155));
        stroke(self.colour); #stroke(255, 153)
        ellipse(self.x, self.y, self.diameter-i*30, self.diameter-i*30);
